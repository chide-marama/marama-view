...
\begin{document}
    ...
    The one that is selected will be the one that will be added to the world.
    These m-blocks are already loaded as assets in the game background, so that makes i quite simple to add these.
    The world contains an array of all the instances that are currently in the world.
    Adding a m-block is as easy as creating a new instance and adding this instance to the array.
    ...
    Object Deletion is handled through an InputController, which libgdx uses to catch events from pointer behaviour.
    When deleting an object, the user selects an object using a boundingbox and a ray on the press of a button. This process is explained above: \href{\clickup{2e29e}}{\#2e29e}.
    When this occurs the object is turned slightly transparent to function as a preview.
    While an object is selected for deletion, the camera is locked.
    A second click will delete the object by removing it from the list of instances in the world.
    If the click hits another object, that object becomes selected instead.
    ...
    Using the bounding box you can find which object you are close to fairly easily.
    However, there is no functionality in libGDX for finding the face of an object.
    To remedy this a new addition was made to the SelectableInstance: Faces.
    Every SelectableInstance has a list of Faces/Joints which contain a vector3 which states the location of the joints.
    When the ray wants to interact with an object(See \href{\clickup{2e29e}}{\#2e29e}) it returns the location of the intersect point.
    The points get translated and compared to one another to find the closest face to the intersect point.
    Then the new object can be added.
    The functionality is not wholly completed.
    It uses the face of the target instance and that of the origin instance to position the new object in the correct spot.
